 ...
 * @description
 * The `appApp.auth.interceptor` module which provides:
 *
...
(function () {
	...
	angular
		.module('minhaIgrejaApp.auth.interceptor', [])
	...
	function AuthInterceptor($q, $cookieStore, $location, $timeout) {
		...
		function request(config) {
			...
			if ($cookieStore.get('token')) {
				config.headers.Authorization = 'Bearer ' + $cookieStore.get('token').access_token;
			}
			return config;
		}
